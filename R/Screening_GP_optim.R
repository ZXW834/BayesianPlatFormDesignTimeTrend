...
#' @param grid.length A numeric value indicating the grid resolution. Default is 5000 for symmetric boundary. For asymmetric boundary, the length of grid is 101 for both efficacy grid and futility grid.
...
#' @param y.pow A numeric vector of power data. Default is NA. Only used when Boundary.type == "Asymmetric"
...
I recommand use grid.length = 101 which is enough for the accuracy."
  ...
  if (Boundary.type == "Symmetric"){
    ...
    sigma.t1E = tau_squared.t1E * (KXX - KX %*% Ki %*% t(KX))
    sd.t1E = sqrt(abs(diag(sigma.t1E)))
    sd.pow = NA
    ...
    e = 1e-10
    weighs = 1 / sqrt((abs(yhat.t1E[target] - errorrate) + e) * abs(diag(sigma.t1E))[which(target)])
  ...
  sigma.pow = tau_squared.pow * (KXX - KX %*% Ki %*% t(KX))
  sd.pow = sqrt(abs(diag(sigma.pow)))
  # construct confidence interval
  ...
  sigma.ESS.null = tau_squared.ESS.null * (KXX - KX %*% Ki %*% t(KX))
  sd.ESS.null = sqrt(abs(diag(sigma.ESS.null)))
  # construct confidence interval
  ...
  sigma.ESS.alt = tau_squared.ESS.alt * (KXX - KX %*% Ki %*% t(KX))
  sd.ESS.alt = sqrt(abs(diag(sigma.ESS.alt)))
  # construct confidence interval
